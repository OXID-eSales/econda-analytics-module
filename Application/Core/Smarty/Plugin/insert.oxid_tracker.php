...
function smarty_insert_oxid_tracker($params, &$smarty)
{
    ...
    if ($config->getConfigParam('blOeEcondaAnalyticsTracking')) {
        $factory = oxNew(\OxidEsales\EcondaAnalyticsModule\Application\Factory::class);

        $entity = $factory->makeActivePageEntityPreparator($smarty)->prepareEntity($params);
        $trackingCodeGenerator = $factory->makeTrackingCodeGenerator($entity);
